...
class PathologyBookingController extends Controller
{
    public function index()
    {
        ...
        return view('admin.pathology_bookings.index', compact('doctors', 'tests'));
    }

    public function show(string $id)
    {
        $booking = DiagnosticBooking::where('type', 'path')->with(['items.diagnostic', 'doctor', 'referredDoctor', 'user'])->findOrFail($id);
        return view('admin.pathology_bookings.show', compact('booking'));
    }
    ...
    public function updateStatus(Request $request)
    {
        ...
        $booking = DiagnosticBooking::where('type', 'path')->findOrFail($request->id);
    ...
    public function uploadReport(Request $request)
    {
        ...
        if ($request->hasFile('report')) {
            if ($booking->report_pdf_path && file_exists(public_path('reports/pathology/' . $booking->report_pdf_path))) {
                unlink(public_path('reports/pathology/' . $booking->report_pdf_path));
            }

            $file = $request->file('report');
            $filename = 'report_' . $booking->booking_no . '_' . time() . '.pdf';
            $destinationPath = public_path('reports/pathology');
        ...
        return redirect()->back()->with('success', 'Pathology report uploaded and marked as Completed!');
    }
    ...
    public function data(Request $request)
    {
        ...
        $query = DiagnosticBooking::where('type', 'path')->select('id', 'booking_no', 'patient_name', 'mobile', 'booking_date', 'booking_time', 'total_amount', 'booking_status', 'payment_status');
        ...
        foreach ($results as $value) {
            $showUrl = route('admin.pathology-bookings.show', $value->id);
            $action = '
